...
var Accordion = function($node) {
    var toggleSelector = '.js_toggle',
        ...
        hide = function($nodes) {
            // $nodes.find('span').addClass('glyphicon glyphicon-plus');
            $nodes.each(function() {
                var $toggle = $(this),
                    $content = $toggle.next(contentSelector);
                $toggle.find('span').removeClass('glyphicon-minus');
    ...
    $node.on('click', ['>', toggleSelector].join(''), function() {
        ...
        if ($el.hasClass(active)) {
            ...
            $el.find('span').removeClass('glyphicon-minus');
        } else {
            show($el);
            $el.find('span').addClass('glyphicon-minus');
        }
